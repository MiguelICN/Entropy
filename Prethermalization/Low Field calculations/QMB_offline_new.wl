...
ExpandBlockVector::usage = "ExpandBlockVector[v, map, type, l] expands a projected parity sector vector back to the full Hilbert space using optimized SparseArrays.";
ProjectBlockVector::usage = "ProjectBlockVector[state, map, type, l] projects a full state vector into the reduced parity basis (inverse of Expand).";
revIndex::usage = "revIndex[i, l] returns the integer corresponding to the bit-reversal of i (for spatial reflection symmetry).";
ExpandFullState::usage = "ExpandFullState[evenState, oddState, evenMap, oddMap, l] reconstructs the full state by coherently summing the expanded even and odd sectors.";
makeExpansionMatrix::usage = "makeExpansionMatrix[map, l, type] returns the sparse isometry matrix mapping the sector basis to the full basis.";
buildExpandFullStateFast::usage = "buildExpandFullStateFast[evenMap, oddMap, l] compiles a high-speed function for repeatedly expanding states during time evolution loops.";
compiledEvolverSector::usage = "compiledEvolverSector[eigenvec, eigenval] returns a WVM-compiled function that evolves a state vector within a specific sector.";
newb::usage = "newb[psit] computes the Entanglement Entropy of a state vector via SVD. It automatically reshapes the vector for an equal bipartite cut.";
new::usage = "new[colVec, l] is a legacy wrapper for newb.";
buildMaps::usage = "buildMaps[l] generates the {evenMap, oddMap} lists required for sector projection.";
pageEntropy::usage = "pageEntropy[La, Lb] returns the theoretical average entanglement entropy (Page value) for a random state.";
...
(* --- State Projection and Expansion (Optimized) --- *)
...
(* --- Entropy Calculation (SVD Method) --- *)
...
newb[psit_] := Module[{d = Round[Sqrt[Length[psit]]], sv2},
  ...
  (* Filter small machine noise to avoid Log[0] or negative zeros *)
  sv2 = Pick[sv2, UnitStep[sv2 - 1.*^-15], 1];
  -sv2 . Log[sv2]
 ]

ClearAll[new]
new[colVec_, l_] := newb[colVec]
...
ClearAll[pur]
pur[L_, La_, Lb_, ini_, eigenval_, eigenvec_, t_] := Module[{state, rho, dima = 2^(La), dimb = 2^(Lb)},
  (* Note: You must define StateEvolution or use compiledEvolverSector here *)
  (* Assuming standard reconstruction is handled externally or StateEvolution exists *)
  state = arrayt[t, eigenval, eigenvec . ini]; (* Simplified placeholder if not using sector evolver *)
  rho = MatrixPartialTrace[Dyad[state], 2, {dima, dimb}];
  Tr[Chop[rho . rho]]
 ]

ClearAll[decoherence]
